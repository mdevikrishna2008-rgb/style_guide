...
def get_single_suggestion(options_list):
    if not options_list:
        return "N/A"
    suggestion = options_list[0]
    if ". Try:" in suggestion:
        suggestion = suggestion.split(". Try:")[0]
    if ". Avoid:" in suggestion:
        suggestion = suggestion.split(". Avoid:")[0]
    return suggestion
...
def generate_suggestions(main_cat, sub_cat, dress_preference, body_type, skin_color, undertone, climate, event_time):
    ...
    # Generate single, concise suggestions for Shoes, Hair, and Accessories
    single_shoe_suggestion = get_single_suggestion(shoes_options)
    single_hair_suggestion = get_single_suggestion(hair_options)
    single_accessory_suggestion = get_single_suggestion(accessories_options)


    color_sugs = generate_color_suggestions(skin_color, undertone)
    
    # Outfit and Makeup still use lists with Try/Avoid advice
    outfit_str = "\n".join(f"{i+1}. {opt}" for i, opt in enumerate(outfit_options))
    makeup_str = "\n".join(f"{i+1}. {opt}" for i, opt in enumerate(makeup_options))

    # Shoes, Hair, and Accessories are now single, direct suggestions
    shoes_str = f"**Stylist's Recommendation:** {single_shoe_suggestion}"
    hair_str = f"**Stylist's Recommendation:** {single_hair_suggestion}"
    accessories_str = f"**Stylist's Recommendation:** {single_accessory_suggestion}"
...
# Body Type (DESCRIPTIONS REMOVED)
...
if st.button("Get My Complete Personalized Look", use_container_width=True, type="primary"):
    ...
    if not (skin_color and undertone and body_type):
        ...
        # --- Visualization (New Requirement) ---
        st.subheader("Event Popularity (Simulated Data)")
        st.markdown(f"The graph below shows the general popularity of event categories compared to your current selection, **{main_cat_key.capitalize()}**.")

        # 1. Create Simulated Data (Proxy for "how often a user selects")
        data = {
            'Category': list(categories.keys()),
            'User Preference (Simulated Count)': [450, 300, 250, 150, 100, 50]
        }
        df = pd.DataFrame(data)

        # 2. Highlight the Current Selection
        df['Color'] = df['Category'].apply(lambda x: '#e84c3d' if x == main_cat_key else '#3498db')
        
        # 3. Create the Bar Chart
        st.bar_chart(
            df.set_index('Category'),
            color=['#3498db'], # Use a single color for simplicity with Streamlit's default bar_chart
            height=300
        )
        st.markdown(f"***Note:*** Your selection ({main_cat_key.capitalize()}) is included in this distribution.")
        
        st.markdown("---")
        
        # --- Display Stylist Advice ---
        st.subheader("Stylist's Detailed Advice")
